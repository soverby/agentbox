...
check "managed-settings.json root 0644, ro" bash -c "
  o=\$(docker exec $run_id stat -c '%U:%G %a' /etc/claude-code/managed-settings.json) && echo \$o &&
  [ \"\$o\" = 'root:root 644' ] &&
  ! docker exec $run_id sh -c 'test -w /etc/claude-code/managed-settings.json' &&
  ! docker exec $run_id sh -c ': >> /etc/claude-code/managed-settings.json' 2>/dev/null"
check "managed-settings.json content" bash -c "
  docker exec $run_id jq -e '.strictKnownMarketplaces==[] and .env.CLAUDE_CODE_DISABLE_OFFICIAL_MARKETPLACE_AUTOINSTALL==\"1\"' /etc/claude-code/managed-settings.json &&
  [ \"\$(docker exec $run_id printenv CLAUDE_CODE_DISABLE_OFFICIAL_MARKETPLACE_AUTOINSTALL)\" = 1 ]"
