...
- Backends behind one interface (`get(ref)`): `keychain` (macOS `security`,
  zero install, default), `op` (1Password CLI; service account per profile,
  scoped to one vault), `env` (tests only). `bws` and Linux `secret-tool`
  later.
...
- Per-box random tokens (router master key, `MCP_GATEWAY_TOKEN`) are
  generated by the CLI at `up` and delivered the same way.
...
| P4 | Secrets backends, scopes, target inference, delivery, `with-secrets` wiring, `setup`, `login codex\|pi`, `secret` commands, git credential setup | Doctor 11 and 17 pass; unit tests per backend and scope; no secret in state dir, `docker inspect`, image layers, or logs; with the user: `setup` stores the Claude token, `login codex` and `login pi` (paste flow) complete, `git push` to a test repo works |
