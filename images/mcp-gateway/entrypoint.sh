...
# then exec the gateway. Values are never printed. gateway.py removes these
# names from its own env at start, so stdio servers never inherit them.
...
if [ -d /run/secrets ]; then
  for __s_f in /run/secrets/*; do
    ...
    case $__s_n in
      [A-Za-z_]*) ;;
      *) echo "mcp-gateway: bad secret name $__s_n" >&2; exit 2 ;;
    esac
    case $__s_n in *[!A-Za-z0-9_]*) echo "mcp-gateway: bad secret name $__s_n" >&2; exit 2 ;; esac
...
if [ "$__s_skip" -gt 0 ]; then
  echo "mcp-gateway: skipped $__s_skip secret(s) with a reserved name" >&2
fi
