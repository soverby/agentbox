...
def load_config(data: Any) -> dict[str, dict]:
    ...
    if not isinstance(data, dict) or set(data) - {"servers"}:
        raise ConfigError("config must be an object with only 'servers'")
...
def upstream_transport(cfg: dict, env: dict[str, str]):
...
def build(
    ...
    for name, cfg in servers.items():
        ...
        if target is None:
            target = Client(upstream_transport(cfg, env))
...
async def probe_one(name: str, cfg: dict, env: dict[str, str], timeout: float) -> dict:
    """Connect to one upstream with a fresh client and list its tools."""
    import asyncio

    from fastmcp import Client

    secrets = [env.get(cfg["bearer_env"] or "", "")]
    try:
        async with asyncio.timeout(timeout):
            async with Client(upstream_transport(cfg, env)) as c:
...
async def probe(servers: dict[str, dict], env: dict[str, str], timeout: float) -> dict:
    import asyncio

    names = list(servers)
    res = await asyncio.gather(*(probe_one(n, servers[n], env, timeout) for n in names))
    return {
...
def main_probe() -> int:
    ...
    servers = load(os.environ.get("MCP_GATEWAY_CONFIG", CONFIG))
    bearers = [s["bearer_env"] for s in servers.values() if s["bearer_env"]]
    env = {**os.environ, **secrets_from_dir(bearers)}
    status = asyncio.run(probe(servers, env, float(os.environ.get("MCP_PROBE_TIMEOUT",
                                                                   PROBE_TIMEOUT))))  # fmt: skip
    write_status(os.environ.get("MCP_GATEWAY_STATUS", STATUS_FILE), status)
    print(json.dumps(status))
    return 0


def startup_probe(servers: dict[str, dict], env: dict[str, str]) -> None:
    ...
    def run():
        try:
            write_status(os.environ.get("MCP_GATEWAY_STATUS", STATUS_FILE),
                         asyncio.run(probe(servers, env, PROBE_TIMEOUT)))  # fmt: skip
...
def main() -> int:
    ...
    try:
        ...
        return 2
    bearers = [s["bearer_env"] for s in servers.values() if s["bearer_env"]]
    secrets = take_env(os.environ, [TOKEN_ENV, *bearers])
